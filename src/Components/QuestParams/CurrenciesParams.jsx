...
const CurrenciesParams = () => {
  ...
  const [deathCount, setDeathCount] = useState(() =>
    ReadDeathCount(questDataView)
  );
  const [mapVariant, setMapVariant] = useState(() =>
    ReadMapVariant(questDataView)
  );
  const [mandatory, setMandatory] = useState(() =>
    ReadMandatoryFlag(questDataView)
  );
    ...
    <Panel onSave={() => onSave()}>
      ...
      <div className="flex gap-x-3">
        <div className="">
          <NumeralInput
            label="Quest Fee"
            defaultValue={questFee}
            onChange={(value) => setQuestFee(parseInt(value))}
          />
        </div>
        <div className="">
          <NumeralInput
            label="Death Count"
            defaultValue={deathCount}
            onChange={(value) => setDeathCount(parseInt(value))}
          />
        </div>
        <div className="">
          <NumeralInput
            label="Map Variant"
            defaultValue={mapVariant}
            onChange={(value) => setMapVariant(parseInt(value))}
          />
        </div>
        <div className="">
          <SelectComponent
            ...
            onChange={(value) => setMandatory(parseInt(value))}
            options={mandatoryDropDownOptions}
          />
        </div>
      </div>
    </Panel>
